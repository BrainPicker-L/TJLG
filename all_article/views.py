...
from .models import Article,TJLG_Article_Pyspider
...
def article_list_shetuan(request):
...
def article_list_jiaowu(request):
    page = request.GET.get('page', '')
    articles_all_list = TJLG_Article_Pyspider.objects.order_by("-created_time")
    paginator = Paginator(articles_all_list, 5)
    if int(page) <= paginator.num_pages:
        if page != '':
            context = {}
